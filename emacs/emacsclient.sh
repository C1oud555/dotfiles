...
emacsfun() {
  ...
  *) cmd="(delete 't (mapcar 'framep (frame-list)))" ;; # if != nil, there are graphical terminals (x, w32, ns)
  ...
  if [ -z "$frames" -o "$frames" = nil ]; then
    emacsclient --alternate-editor="" --create-frame "$@" -e "(select-frame-set-input-focus (selected-frame))"
    return $?
  fi

  emacsclient --alternate-editor="" "$@" -e "(select-frame-set-input-focus (selected-frame))"
}
